    ...
    <div class="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition relative group max-w-sm">
        <div class="flex justify-end items-end mb-4">
            <span class="text-slate-400 text-[10px] flex items-center bg-slate-50 px-2 py-1 rounded-lg">
                <i class="far fa-clock mr-1"></i> {{ $offer->created_at->diffForHumans() }} ago
            </span>
        </div>
        ...
        <div class="flex items-center justify-between gap-4 mb-6">
            <div class="flex flex-col items-center flex-1">
                <div class="relative w-24 h-24 rounded-full overflow-hidden border-2 border-slate-100 shadow-inner">
                    <img src="{{asset('storage/'.$offer->component->first()->image)}}" alt="Turkey Sandwich" class="w-full h-full object-cover">
                </div>
    ...
    <div class="bg-white w-full max-w-4xl rounded-[3rem] shadow-2xl overflow-hidden mx-4">
        ...
        <div class="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
            <div>
                ...
                <p class="text-sm text-slate-500 font-medium">Select an item from your tray to trade for the <span class="text-orange-600 font-bold">{{$offer->component->name}}</span>.</p>
            </div>
        ...
        <form action="{{route('swaps.store',$offer->id)}}" method="POST">
            @csrf
            <div class="p-8">
